          ...
          return (
            <div key={i} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-5">
              <div className="flex items-start justify-between gap-3">
                <p dir="rtl" className="text-2xl text-emerald-800 dark:text-emerald-300 leading-relaxed flex-1">
                  {item.arabic}
                </p>
                {supported && (
                  <button
                    onClick={() => speak(itemId, item.arabic)}
                    className="text-xs text-emerald-700 dark:text-emerald-300 hover:underline shrink-0 mt-1"
                  >
                    {speakingId === itemId ? '⏸ Pause' : '▶ Listen'}
                  </button>
                )}
              </div>
              <p className="text-gray-600 dark:text-gray-300 mt-3 text-sm italic">{item.translation}</p>
              {user && (
                <div className="flex items-center gap-3 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                  ...
                  {(counts[itemId] || 0) > 0 && (
                    <button
                      ...
                      className="text-xs text-gray-400 dark:text-gray-400 hover:text-gray-600 dark:text-gray-300"
                    >
                      Reset
                    </button>
                  )}
