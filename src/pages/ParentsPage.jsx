...
function StudentCard({ student, isAdmin, onRemove }) {
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-5 flex items-start gap-4 shadow-sm">
      <div className="w-14 h-14 rounded-full bg-emerald-100 dark:bg-gray-700 text-emerald-700 dark:text-emerald-300 flex items-center justify-center text-xl font-bold shrink-0">
      ...
      <div className="flex-1 text-left">
        ...
        <p className="text-emerald-700 dark:text-emerald-300 text-sm font-medium">{student.grade}</p>
        {student.className && <p className="text-gray-500 dark:text-gray-400 text-sm">Class: {student.className}</p>}
        {student.school && <p className="text-gray-500 dark:text-gray-400 text-sm">{student.school}</p>}
        {student.notes && <p className="text-gray-400 dark:text-gray-400 text-xs mt-1 italic">{student.notes}</p>}
        {isAdmin && (
          <p className="text-xs text-gray-400 dark:text-gray-400 mt-2">Parent: {student.parentEmail}</p>
        )}
      </div>
      {isAdmin && (
        <button
...
export default function ParentsPage() {
  ...
  return (
    <section className="scroll-mt-20 bg-emerald-50 dark:bg-gray-900 min-h-screen py-16 px-6 md:px-12 text-center">
      <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">👨‍👩‍👧‍👦 Parents Portal</h2>
      ...
      {!user ? (
        <div className="mt-10">
          <p className="text-gray-600 dark:text-gray-300 mb-4">Sign in with the email you were invited with</p>
          <button
        ...
        <div className="mt-8 max-w-2xl mx-auto">
          ...
          {loading && <p className="text-gray-400 dark:text-gray-400 text-sm">Loading students...</p>}

          {!loading && (
            <>
              <h3 className="text-left text-lg font-bold text-gray-700 dark:text-gray-200 mb-3">
                {isAdmin ? 'All Students' : 'My Students'}
              </h3>
              {students.length === 0 ? (
                <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-8 text-gray-400 dark:text-gray-400 text-sm">
                  {isAdmin
          ...
          {isAdmin && (
            <form
              ...
              className="mt-10 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-6 text-left space-y-3 shadow-sm"
            >
              <h3 className="text-lg font-bold text-gray-700 dark:text-gray-200">➕ Add a Student (invite a parent)</h3>
              <input
                type="text"
                required
